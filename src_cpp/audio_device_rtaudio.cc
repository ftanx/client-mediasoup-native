...
#include "audio_device_rtaudio.h"
...
void RtAudioDevice::AtomicSet32(int32_t* theValue, int32_t newValue) {
  while (1) {
...
int32_t RtAudioDevice::AtomicGet32(int32_t* theValue) {
  while (1) {
...
void RtAudioDevice::logCAMsg(const rtc::LoggingSeverity sev,
                              const char* msg,
                              const char* err) {
  RTC_DCHECK(msg != NULL);
  RTC_DCHECK(err != NULL);
...
RtAudioDevice::RtAudioDevice()
  ...
  RTC_LOG(LS_INFO) << __FUNCTION__ << " created";
	...
	std::cout << "YES IT IS USED!" << std::endl;
...
void RtAudioDevice::AttachAudioBuffer(AudioDeviceBuffer* audioBuffer) {
...
int32_t RtAudioDevice::ActiveAudioLayer(
    AudioDeviceModule::AudioLayer& audioLayer) const {
  audioLayer = AudioDeviceModule::kPlatformDefaultAudio;
  return 0;
}

AudioDeviceGeneric::InitStatus RtAudioDevice::Init() {
  rtc::CritScope lock(&_critSect);
...
int32_t RtAudioDevice::Terminate() {
  if (!_initialized) {
    return 0;
  }
...
bool RtAudioDevice::Initialized() const {
  return (_initialized);
}

int32_t RtAudioDevice::SpeakerIsAvailable(bool& available) {
...
int32_t RtAudioDevice::InitSpeaker() {
  rtc::CritScope lock(&_critSect);
...
int32_t RtAudioDevice::MicrophoneIsAvailable(bool& available) {
...
int32_t RtAudioDevice::InitMicrophone() {
  rtc::CritScope lock(&_critSect);
...
bool RtAudioDevice::SpeakerIsInitialized() const {
  return (_mixerManager.SpeakerIsInitialized());
}

bool RtAudioDevice::MicrophoneIsInitialized() const {
  return (_mixerManager.MicrophoneIsInitialized());
}

int32_t RtAudioDevice::SpeakerVolumeIsAvailable(bool& available) {
  bool wasInitialized = _mixerManager.SpeakerIsInitialized();
...
int32_t RtAudioDevice::SetSpeakerVolume(uint32_t volume) {
  return (_mixerManager.SetSpeakerVolume(volume));
}

int32_t RtAudioDevice::SpeakerVolume(uint32_t& volume) const {
  uint32_t level(0);
...
int32_t RtAudioDevice::MaxSpeakerVolume(uint32_t& maxVolume) const {
  uint32_t maxVol(0);
...
int32_t RtAudioDevice::MinSpeakerVolume(uint32_t& minVolume) const {
  uint32_t minVol(0);
...
int32_t RtAudioDevice::SpeakerMuteIsAvailable(bool& available) {
...
int32_t RtAudioDevice::SetSpeakerMute(bool enable) {
  return (_mixerManager.SetSpeakerMute(enable));
}

int32_t RtAudioDevice::SpeakerMute(bool& enabled) const {
  bool muted(0);
...
int32_t RtAudioDevice::MicrophoneMuteIsAvailable(bool& available) {
  bool isAvailable(false);
  bool wasInitialized = _mixerManager.MicrophoneIsInitialized();
...
int32_t RtAudioDevice::SetMicrophoneMute(bool enable) {
  return (_mixerManager.SetMicrophoneMute(enable));
}

int32_t RtAudioDevice::MicrophoneMute(bool& enabled) const {
  bool muted(0);
...
int32_t RtAudioDevice::StereoRecordingIsAvailable(bool& available) {
  bool isAvailable(false);
  bool wasInitialized = _mixerManager.MicrophoneIsInitialized();
...
int32_t RtAudioDevice::SetStereoRecording(bool enable) {
  if (enable)
    _recChannels = 2;
  else
    _recChannels = 1;
...
int32_t RtAudioDevice::StereoRecording(bool& enabled) const {
  if (_recChannels == 2)
    enabled = true;
  else
    enabled = false;
...
int32_t RtAudioDevice::StereoPlayoutIsAvailable(bool& available) {
  bool isAvailable(false);
  bool wasInitialized = _mixerManager.SpeakerIsInitialized();
...
int32_t RtAudioDevice::SetStereoPlayout(bool enable) {
  if (enable)
    _playChannels = 2;
  else
    _playChannels = 1;
...
int32_t RtAudioDevice::StereoPlayout(bool& enabled) const {
  if (_playChannels == 2)
    enabled = true;
  else
    enabled = false;
...
int32_t RtAudioDevice::MicrophoneVolumeIsAvailable(bool& available) {
  bool wasInitialized = _mixerManager.MicrophoneIsInitialized();
...
int32_t RtAudioDevice::SetMicrophoneVolume(uint32_t volume) {
  return (_mixerManager.SetMicrophoneVolume(volume));
}

int32_t RtAudioDevice::MicrophoneVolume(uint32_t& volume) const {
  uint32_t level(0);
...
int32_t RtAudioDevice::MaxMicrophoneVolume(uint32_t& maxVolume) const {
  uint32_t maxVol(0);
...
int32_t RtAudioDevice::MinMicrophoneVolume(uint32_t& minVolume) const {
  uint32_t minVol(0);
...
int32_t RtAudioDevice::SetPlayoutDevice(
    AudioDeviceModule::WindowsDeviceType /*device*/) {
  RTC_LOG(LS_ERROR) << "WindowsDeviceType not supported";
  return -1;
}

int32_t RtAudioDevice::PlayoutDeviceName(uint16_t index,
...
int32_t RtAudioDevice::RecordingDeviceName(uint16_t index,
                                            char name[kAdmMaxDeviceNameSize],
                                            char guid[kAdmMaxGuidSize]) {
...
int32_t RtAudioDevice::SetRecordingDevice(
    AudioDeviceModule::WindowsDeviceType /*device*/) {
  RTC_LOG(LS_ERROR) << "WindowsDeviceType not supported";
  return -1;
}

int32_t RtAudioDevice::PlayoutIsAvailable(bool& available) {
  available = true;
...
int32_t RtAudioDevice::RecordingIsAvailable(bool& available) {
  available = true;
...
int32_t RtAudioDevice::InitPlayout() {
...
int32_t RtAudioDevice::InitRecording() {
  ...
  _captureLatencyUs = (UInt32)((1.0e6 * latency) / _inStreamFormat.mSampleRate);
	...
	std::cout << "Latency is: " +  _captureLatencyUs << std::endl;
...
int32_t RtAudioDevice::StartRecording() {
...
int32_t RtAudioDevice::StopRecording() {
...
bool RtAudioDevice::RecordingIsInitialized() const {
  return (_recIsInitialized);
}

bool RtAudioDevice::Recording() const {
  return (_recording);
}

bool RtAudioDevice::PlayoutIsInitialized() const {
  return (_playIsInitialized);
}

int32_t RtAudioDevice::StartPlayout() {
...
int32_t RtAudioDevice::StopPlayout() {
...
int32_t RtAudioDevice::PlayoutDelay(uint16_t& delayMS) const {
...
bool RtAudioDevice::Playing() const {
  return (_playing);
}
...
int32_t RtAudioDevice::GetNumberDevices(const AudioObjectPropertyScope scope,
...
int32_t RtAudioDevice::GetDeviceName(const AudioObjectPropertyScope scope,
...
int32_t RtAudioDevice::InitDevice(const uint16_t userDeviceIndex,
...
OSStatus RtAudioDevice::SetDesiredPlayoutFormat() {
  // Our preferred format to work with.
...
OSStatus RtAudioDevice::objectListenerProc(
    AudioObjectID objectId,
    UInt32 numberAddresses,
    const AudioObjectPropertyAddress addresses[],
    void* clientData) {
  RtAudioDevice* ptrThis = (RtAudioDevice*)clientData;
...
OSStatus RtAudioDevice::implObjectListenerProc(
    const AudioObjectID objectId,
    const UInt32 numberAddresses,
    const AudioObjectPropertyAddress addresses[]) {
  RTC_LOG(LS_VERBOSE) << "RtAudioDevice::implObjectListenerProc()";
...
int32_t RtAudioDevice::HandleDeviceChange() {
  OSStatus err = noErr;
...
int32_t RtAudioDevice::HandleStreamFormatChange(
    const AudioObjectID objectId,
...
int32_t RtAudioDevice::HandleDataSourceChange(
    const AudioObjectID objectId,
    const AudioObjectPropertyAddress propertyAddress) {
  OSStatus err = noErr;
...
int32_t RtAudioDevice::HandleProcessorOverload(
    const AudioObjectPropertyAddress propertyAddress) {
...
OSStatus RtAudioDevice::deviceIOProc(AudioDeviceID,
...
OSStatus RtAudioDevice::outConverterProc(AudioConverterRef,
                                          UInt32* numberDataPackets,
                                          AudioBufferList* data,
                                          AudioStreamPacketDescription**,
                                          void* userData) {
  RtAudioDevice* ptrThis = (RtAudioDevice*)userData;
  RTC_DCHECK(ptrThis != NULL);

  return ptrThis->implOutConverterProc(numberDataPackets, data);
}

OSStatus RtAudioDevice::inDeviceIOProc(AudioDeviceID,
                                        const AudioTimeStamp*,
                                        const AudioBufferList* inputData,
                                        const AudioTimeStamp* inputTime,
                                        AudioBufferList*,
...
OSStatus RtAudioDevice::inConverterProc(
    AudioConverterRef,
    UInt32* numberDataPackets,
    AudioBufferList* data,
    AudioStreamPacketDescription** /*dataPacketDescription*/,
    void* userData) {
  RtAudioDevice* ptrThis = static_cast<RtAudioDevice*>(userData);
  RTC_DCHECK(ptrThis != NULL);

  return ptrThis->implInConverterProc(numberDataPackets, data);
}

OSStatus RtAudioDevice::implDeviceIOProc(const AudioBufferList* inputData,
                                          const AudioTimeStamp* inputTime,
                                          AudioBufferList* outputData,
                                          const AudioTimeStamp* outputTime) {
  OSStatus err = noErr;
...
OSStatus RtAudioDevice::implOutConverterProc(UInt32* numberDataPackets,
                                              AudioBufferList* data) {
...
OSStatus RtAudioDevice::implInDeviceIOProc(const AudioBufferList* inputData,
                                            const AudioTimeStamp* inputTime) {
  OSStatus err = noErr;
...
OSStatus RtAudioDevice::implInConverterProc(UInt32* numberDataPackets,
                                             AudioBufferList* data) {
  RTC_DCHECK(data->mNumberBuffers == 1);
  PaRingBufferSize numSamples =
...
void RtAudioDevice::RunRender(void* ptrThis) {
  RtAudioDevice* device = static_cast<RtAudioDevice*>(ptrThis);
  while (device->RenderWorkerThread()) {
  }
}

bool RtAudioDevice::RenderWorkerThread() {
  PaRingBufferSize numSamples =
...
void RtAudioDevice::RunCapture(void* ptrThis) {
  RtAudioDevice* device = static_cast<RtAudioDevice*>(ptrThis);
  while (device->CaptureWorkerThread()) {
  }
}

bool RtAudioDevice::CaptureWorkerThread() {
  OSStatus err = noErr;
  UInt32 noRecSamples =
